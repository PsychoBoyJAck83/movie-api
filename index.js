...
app.post('/users/:username/favorites/:movieId', passport.authenticate('jwt', { session: false }), (req, res) => {
	if (req.params.movieId.length != 24) res.status(400).send('invalid movieId');
	else {
		Movies.findById(req.params.movieId).then((movie) => {
			if (!movie) res.status(400).send('Movie ID ' + req.params.movieId + ' is unknown.');
			else {
				Users.findOneAndUpdate(
					{ Username: req.params.username },
					...
					if (!user) res.status(400).send('User ' + req.params.username + ' is unknown.');
...
app.delete('/users/:username', passport.authenticate('jwt', { session: false }), (req, res) => {
	Users.findOneAndDelete({ Username: req.params.username })
		.then((user) => {
			if (!user) res.status(400).send('User ' + req.params.username + ' not found!');
